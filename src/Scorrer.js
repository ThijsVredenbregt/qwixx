	...
	return (
		<div className="row-container" style={{backgroundColor:Props.color}}>
			<div className="checkbox-container" style={{backgroundColor: Props.darkColor}}>
				{Props.items.map((item, index) => <ScorrerCheckbox key={index} lightColor={Props.lightColor} index={index} value={item.value} row={Props.row} checked={item.checked} disabled={item.disabled} onClickHandler={Props.onClickHandler} />)}
			</div>
